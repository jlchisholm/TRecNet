...
def appendCalculations(df,model_name):
    ...
    for level in levels:
        ...
        if 'Chi2' not in model_name:
            ...
            df[level+'_b1_px'] = df[level+'_b1_pt']*np.cos(df[level+'_b1_phi'])
            df[level+'_b1_py'] = df[level+'_b1_pt']*np.sin(df[level+'_b1_phi'])
            df[level+'_b2_px'] = df[level+'_b2_pt']*np.cos(df[level+'_b2_phi'])
            df[level+'_b2_py'] = df[level+'_b2_pt']*np.sin(df[level+'_b2_phi'])
...
def createDirectories(main_dir):
    ...
    if not os.path.exists(main_dir):
        os.mkdir(main_dir)
    for par_dir in ['th/','tl/','ttbar/', 'b1/', 'b2/']:
