...
def appendCalculations(df,model_name):
    ...
    for level in levels:
        ...
        if 'Chi2' not in model_name:
            ...
            df[level+'_b_px'] = df[level+'_b_pt']*np.cos(df[level+'_b_phi'])
            df[level+'_b_py'] = df[level+'_b_pt']*np.sin(df[level+'_b_phi'])
            df[level+'_bbar_px'] = df[level+'_bbar_pt']*np.cos(df[level+'_bbar_phi'])
            df[level+'_bbar_py'] = df[level+'_bbar_pt']*np.sin(df[level+'_bbar_phi'])
...
def createDirectories(main_dir):
    ...
    if not os.path.exists(main_dir):
        os.mkdir(main_dir)
    for par_dir in ['th/','tl/','ttbar/', 'b/', 'bbar/']:
