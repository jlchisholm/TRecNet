...
class ttbbCutter:
    ...
    def cutTree(self, tree, num_b_tags, max_b_tags, n_jets_min, DL1r_op_point):
        """
        Implements cuts on input tree (from root file) on the minimum number of b tags and minimum number of jets
            ...
            Parameters:
                ...
                max_b_tags (int): maximum number of b tags (optional)
                DL1r_op_point (int in 0-5): DL1r operating point
        ...
        if max_b_tags: b_max_sel = [(sum(weights)<= num_b_tags) for weights in b_tags] # add up the binary value of b-tags per event and check if <= num_b_tags
        ...
        if max_b_tags: sel=np.logical_and(sel, b_max_sel)
        tree = tree[sel]
    ...
    def ttbbCut(self, root_file, save_dir, tree_name, num_b_tags, max_b_tags, n_jets_min, DL1r_op_point, no_name_change):
        """
        ...
        # add binary "is be tagged" branch if not already included (currently hardcoded to 70% operating point to match Jenna's TRecNet setup)
        ...
        # cut tree events
        print('cutting tree')
        nom_tree = self.cutTree(nom_tree, num_b_tags, max_b_tags, n_jets_min, DL1r_op_point)
...
class truthPrep:
    ...
    def switchTTBarToHadLep(self, tree, keys):
        ...
        # Get truth information for tops (after FSR)
    ...
    def addExtras(self, tree, keys):
        ...
        # store 4-vector of b and bbar
        b_vec_afterFSR = vector.array({"pt":tree['MC_b_afterFSR_pt'],"eta":tree['MC_b_afterFSR_eta'],"phi":tree['MC_b_afterFSR_phi'],"m":tree['MC_b_afterFSR_m']})
        bbar_vec_afterFSR = vector.array({"pt":tree['MC_bbar_afterFSR_pt'],"eta":tree['MC_bbar_afterFSR_eta'],"phi":tree['MC_bbar_afterFSR_phi'],"m":tree['MC_bbar_afterFSR_m']})
        
        # add dphi for b and bbar
        keys.append('MC_deltaPhi_bb')
        tree['MC_deltaPhi_bb'] = np.absolute(b_vec_afterFSR.deltaphi(bbar_vec_afterFSR))

        # add dR for b and bbar
        keys.append('MC_deltaR_bb')
        tree['MC_deltaR_bb'] = b_vec_afterFSR.deltaR(bbar_vec_afterFSR)

        # add rapidity and energy for b and bbar
        keys.append('MC_b_afterFSR_y')
        keys.append('MC_b_afterFSR_E')
        tree['MC_b_afterFSR_y'] = b_vec_afterFSR.rapidity
        tree['MC_b_afterFSR_E'] = b_vec_afterFSR.E

        keys.append('MC_bbar_afterFSR_y')
        keys.append('MC_bbar_afterFSR_E')
        tree['MC_bbar_afterFSR_y'] = b_vec_afterFSR.rapidity
        tree['MC_bbar_afterFSR_E'] = b_vec_afterFSR.E
...
p_ttbbCut.add_argument('--max_b_tags', help='maximum number of b tags per event', type=int, required=False)
...
if args.function == 'ttbbCut':
    cutter = ttbbCutter()
    cutter.ttbbCut(args.root_file, args.save_dir, args.tree_name, args.num_b_tags, args.max_b_tags, args.n_jets_min, args.DL1r_op_point, args.no_name_change)
