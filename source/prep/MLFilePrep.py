...
class MLFilePrep:
    ...
    def getJetsData(self,reco_tree,jn,var_conf):
        """
    ...
    def getOtherData(self,reco_tree,var_conf):
        """
        ...
        keys = getObservableNamesDict(var_conf,'lep_pt','lep_eta','lep_phi','met_met','met_phi','jet_n')
        df_other = ak.to_dataframe({ML_name:reco_tree[ntuple_name]/1000 if ML_name=='lep_pt' or ML_name=='met_met' else reco_tree[ntuple_name] for ML_name, ntuple_name in keys.items()})
    ...
    def getTruthData(self,truth_tree,jn,ttbb,var_conf):
        """
        ...
        # Get the truth keys for th, tl, and ttbar
        truth_keys = {}
        for p in ['th_','tl_']:
            for v in ['pt','eta','phi','m','y','E','pout']:
                truth_keys[p+v] = getObservableName(var_conf,p+v)
        for v in ['pt','eta','phi','m','y','E','dphi','Ht','chi','yboost']:
            truth_keys['ttbar_'+v] = getObservableName(var_conf,'ttbar_'+v)
                    
        # Get the wh and wl keys
        for p in ['wh_','wl_']:
            for v in ['pt','eta','phi','m']:
                truth_keys[p+v] = getObservableName(var_conf,p+v)
                
        # If ttbb, also get b and bbar keys
        if ttbb:
            for p in ['b_','bbar_']:
                for v in ['pt', 'eta', 'phi', 'm']:
                    truth_keys[p+v] = getObservableName(var_conf,p+v)
        ...
        # Include jet match info
        str_jet_isTruth = getObservableName(var_conf,'jet_isTruth')
        padded_matches = np.asarray(ak.fill_none(ak.pad_none(truth_tree[str_jet_isTruth], jn, clip=True), 0))
        for j in range(jn):
    ...
    def getDataframes(self,root_file,tree_name,jn,met_cut,ttbb,var_conf):
        """
    ...
    def makeH5File(self,input,output,tree_type,jn,ttbb,var_conf):
        """
        ...
        df_jets, df_other, df_truth = self.getDataframes(input,tree_name,jn,ttbb,var_conf)

        # Add a tag for the data type and the number of jets included
        tag = '_sysUP' if tree_type=='up' else '_sysDOWN' if tree_type=='down' else ''
...
if args.function == 'makeH5File':
    prepper = MLFilePrep()
    prepper.makeH5File(args.input,args.output,args.tree_type,args.jn,args.ttbb)
