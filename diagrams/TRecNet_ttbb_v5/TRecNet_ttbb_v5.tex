...
\begin{document}
	...
	% INPUTS
	\node[input] at (\s,2*\h) (jetinput) {Input (Jets)};
	...
	% JET REWEIGHTING
	\draw[draw=bgcolor!40,very thick,fill=bgcolor!40] (5.5,-10.5) rectangle ++(-5,-9.25) node[below right,color=bgcolor!50!black] {Weighted Jet Processor};
	...
	% BB PRETRAINER
	\draw[draw=bgcolor!40,very thick,fill=bgcolor!40] (20.5,-1) node[above left,color=bgcolor!50!black] {bb Classifier} rectangle ++(-5,-9.25);
	\node[hidden] at (6\s,-1.5*\h) (bbflattenedjets) {Flatten};
	\node[hidden] at (6\s,-3*\h) (bbconcatjetsother) {Concatenate};
	\node[hidden] at (6\s,-4.5*\h) (bbdense256-1) {Dense (ReLU, 256)};
	\node[hidden] at (6\s,-6*\h) (bbdense256-2) {Dense (ReLU, 256)};
	\node[hidden] at (6\s,-7.5*\h) (bbjetmatchoutput) {Dense (Sigmoid, n\_jets)};
	
	% B-JET PATH
	\draw[draw=bgcolor!40,very thick,fill=bgcolor!40] (26.5,-1) node[above left,color=bgcolor!50!black] {Initial b-jet Processor} rectangle ++(-5,-7.5);
	\node[hidden] at (8\s,-1.5*\h) (bbmaskingjets) {Masking};
	\node[hidden] at (8\s,-3*\h) (bbreshapemaskedjets) {Reshape};
	\node[hidden] at (8\s,-4.5*\h) (bbTDDense128) {TDDense (ReLU, 128)};
	\node[hidden] at (8\s,-6*\h) (bbTDDense64) {TDDense (ReLU, 64)};
	
	% B-JET REWEIGHTING
	\draw[draw=bgcolor!40,very thick,fill=bgcolor!40] (29.5,-10.5) rectangle ++(-5,-9.25) node[below right,color=bgcolor!50!black] {Weighted b-jet Processor};
	\node[hidden] at (9\s,-9*\h) (bbreshape) {Reshape};
	\node[hidden] at (9\s,-10.5*\h) (bbweightjets) {Multiply};
	\node[hidden] at (9\s,-12*\h) (bbTDDense256-1) {TDDense (ReLU, 256)};
	\node[hidden] at (9\s,-13.5*\h) (bbTDDense256-2) {TDDense (ReLU, 256)};
	\node[hidden] at (9\s,-15*\h) (bbflattenedweightedjets) {Flatten};
	...
	\node[coordinate, left=0.25*\w of bbconcatjetsother] (other-bbclassifier) {};
	\node[coordinate, left=0.25*\w of bbmaskingjets] (bbclassifier-bjet) {};
	\node[coordinate, right=0.25*\w of bbweightjets] (outerbb) {};
	\node[coordinate, right=0.25*\w of bconcat] (right-bconcat) {};
	...
	% BB PRETRAINER
	\draw[-] (jetinput) -| (bbclassifier-bjet);
	\draw[-latex] (bbclassifier-bjet) -- (bbflattenedjets);
	\draw[-latex] (bbflattenedjets) -- (bbconcatjetsother);
	\draw[-] (otherinput) -| (other-bbclassifier);
	\draw[-latex] (other-bbclassifier) -- (bbconcatjetsother);
	\draw[-latex] (bbconcatjetsother) -- (bbdense256-1);
	\draw[-latex] (bbdense256-1) -- (bbdense256-2);
	\draw[-latex] (bbdense256-2) -- (bbjetmatchoutput);
	
	% B-JET PATH
	\draw[-] (jetinput) -| (bbclassifier-bjet);
	\draw[-latex] (bbclassifier-bjet) -- (bbmaskingjets);
	\draw[-latex] (bbmaskingjets) -- (bbreshapemaskedjets);
	\draw[-latex] (bbreshapemaskedjets) -- (bbTDDense128);
	\draw[-latex] (bbTDDense128) -- (bbTDDense64);
	...
	% B-JET REWEIGHTING
	\draw[-] (bbTDDense64) -| (outerbb);
	\draw[-latex] (outerbb) -- (bbweightjets);
	\draw[-latex] (bbjetmatchoutput) -| (bbreshape);
	\draw[-latex] (bbreshape) -- (bbweightjets);
	\draw[-latex] (bbweightjets) -- (bbTDDense256-1);
	\draw[-latex] (bbTDDense256-1) -- (bbTDDense256-2);
	\draw[-latex] (bbTDDense256-2) -- (bbflattenedweightedjets);
	...
	\draw (bbflattenedweightedjets) -| (right-bconcat);
	\draw[-latex] (right-bconcat) -- (bconcat);
