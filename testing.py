    ...
    def test(self, Model, save_loc, ttbb):
        ...
        with h5py.File(self.test_file,'r') as dataset:
            ...
            if self.data_type=='nominal': 
                ...
                else: truth_keys = ['eventNumber','jet_n']+list(filter(lambda a: 'th_' in a or 'tl_' in a or 'ttbar_' in a or 'wh_' in a or 'wl_' in a or 'b_' in a or 'bbar_' in a, dataset.keys()))
